...
var aptos_wasm_exports = {};
__export(aptos_wasm_exports, {
  Address: () => Address,
  Ascii: () => Ascii,
  Boolean: () => Boolean,
  Signer: () => Signer,
  ...
  set_move_toml_addresses: () => set_move_toml_addresses,
  setup: () => setup,
  setup_move: () => setup_move,
  to_arr_value: () => to_arr_value,
  upgrade_move_module: () => upgrade_move_module
});
module.exports = __toCommonJS(aptos_wasm_exports);
...
var bytes = import_fs.default.readFileSync(bytes_dir + "/aptos_wasm_bg.wasm");
      ...
      if (dep_p["git"] && dep_p["subdir"] && dep_p["rev"]) {
        ...
        if (dep_name == STD_NAME) {
          ...
          if (remote_subdir_final) {
            import_fs_extra4.default.writeFileSync(sparse_checkout_path, remote_subdir_final);
          }
        } else {
          if (remote_subdir) {
...
async function clone_chain_move_module(move_gen2, network, module_objectid, package_path) {
  ...
  for (var objectid in object_modules) {
    ...
    import_fs_extra5.default.writeFileSync(out_bcs_file, JSON.stringify(module_bcs, null, 2));
  }
  let toml_path = package_path + "/Move.toml";
...
function setup(runtime, package_path) {
  ...
  for (var dep_module in toml.dependencies) {
    if (dep_module.startsWith("0x") && toml.dependencies[dep_module]["local"]) {
      ...
      if (import_fs_extra6.default.existsSync(local_bcs_path)) {
        let bcs_json_files = import_fs_extra6.default.readdirSync(local_bcs_path);
        for (var i = 0; i < bcs_json_files.length; i++) {
          let bcs_json_file = local_bcs_path + "/" + bcs_json_files[i];
          var bcs_json_data = import_fs_extra6.default.readFileSync(bcs_json_file).toString();
          if (bcs_json_data) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    let bytes2 = import_fs_extra6.default.readFileSync(files[i]);
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    let bytes2 = import_fs_extra6.default.readFileSync(files[i]).toString();
...
function setup_move(runtime, package_path, include_deps) {
  ...
  if (include_deps) {
    ...
    var dirfiles = import_fs_extra6.default.readdirSync(bytecode_path);
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    var bytes2 = import_fs_extra6.default.readFileSync(file);
...
var import_read_dir_deep2 = require("read-dir-deep");
...
function upgrade_move_module(aptos_bin_path, package_path, network, object_address) {
  ...
  console.log(cmd);
  let out = import_shelljs2.default.exec(cmd).stdout;
...
function set_aptos_yaml(package_path, network) {
  ...
  if (!import_fs_extra7.default.existsSync(aptos_path)) {
    import_fs_extra7.default.mkdirpSync(aptos_path);
  }
  ...
  let key = import_ts_sdk2.PrivateKey.formatPrivateKey(private_key, import_ts_sdk2.PrivateKeyVariants.Ed25519);
  const privateKey = new import_ts_sdk2.Ed25519PrivateKey(key);
  const aptos_account = import_ts_sdk2.Account.fromPrivateKey({ privateKey });
  ...
  yaml_string = (0, import_yaml.stringify)(content);
  import_fs_extra7.default.writeFileSync(yaml_path, yaml_string);
}
function set_move_toml_addresses(module_name, package_path, address) {
  ...
  if (addresses[module_name] != address) {
    ...
    let toml_string2 = import_fs_extra7.default.readFileSync(toml_path).toString();
    toml_string2 = MoveGen.toml_edit_addresses(toml_string2, "addresses", module_name, address);
    import_fs_extra7.default.writeFileSync(toml_path, toml_string2);
  }
}
function get_module_name(package_path) {
  let sources_path = package_path + "/sources";
  let files = (0, import_read_dir_deep2.readDirDeepSync)(sources_path, { gitignore: false, ignore: [] });
  for (var i = 0; i < files.length; i++) {
    let move_code = import_fs_extra7.default.readFileSync(files[i]).toString();
