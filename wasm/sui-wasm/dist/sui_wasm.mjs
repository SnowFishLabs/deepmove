...
var bytes = fs2.readFileSync(bytes_dir + "/sui_wasm_bg.wasm");
...
// src/sui/sui_wasm.ts
import { decodeSuiPrivateKey as decodeSuiPrivateKey2 } from "@mysten/sui/cryptography";
    ...
    for (var dep in deps_remotes) {
      ...
      if (dep_p["git"] && dep_p["subdir"] && dep_p["rev"]) {
        ...
        if (dep_name == STD_NAME) {
          fs5.writeFileSync(sparse_checkout_path, "/crates/sui-framework/packages");
        } else {
  ...
  if (results) {
    for (var i = 0; i < results.length; i++) {
      ...
      if (r.data) {
        ...
        fs6.writeFileSync(out_bcs_file, JSON.stringify(moduleMap, null, 2));
      }
    }
    let toml_path = package_path + "/Move.toml";
...
function debug_move_function(wasm_runtime, code_helper, package_path, module_name, func_name) {
  ...
  fs7.removeSync(trace_dir);
  fs7.mkdirpSync(trace_dir);
...
function setup(runtime, package_path) {
  ...
  for (var dep_module in toml.dependencies) {
    if (dep_module.startsWith("0x") && toml.dependencies[dep_module]["local"]) {
      ...
      if (fs8.existsSync(local_bcs_path)) {
        let bcs_json_files = fs8.readdirSync(local_bcs_path);
        for (var i = 0; i < bcs_json_files.length; i++) {
          let bcs_json_file = local_bcs_path + "/" + bcs_json_files[i];
          var bcs_json_data = fs8.readFileSync(bcs_json_file).toString();
          if (bcs_json_data) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    let bytes2 = fs8.readFileSync(files[i]);
  ...
  if (!fs8.existsSync(sourec_map_path)) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    let bytes2 = fs8.readFileSync(files[i]).toString();
...
function setup_move(runtime, package_path, include_deps) {
  ...
  if (include_deps) {
    ...
    var dirfiles = fs8.readdirSync(bytecode_path);
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    var bytes2 = fs8.readFileSync(file);
...
// src/sui/publish.ts
import { getFullnodeUrl as getFullnodeUrl2, SuiClient as SuiClient2 } from "@mysten/sui/client";
...
async function upgrade_move_module(sui_bin_path, package_path, network, old_package_id, upgrade_cap_id) {
  shell2.cd(package_path);
  let move_lock = package_path + "/Move.lock";
  fs9.removeSync(move_lock);
  let buildCommandOutput = shell2.exec(`${sui_bin_path} move build --dump-bytecode-as-base64`).stdout;
  const { modules, dependencies, digest } = JSON.parse(buildCommandOutput);
  const client = new SuiClient2({ url: getFullnodeUrl2(network) });
...
function new_sui_client() {
  ...
  if (network) {
    return new SuiClient3({ url: getFullnodeUrl3(network) });
    ;
  }
  return null;
}
async function sign_execute_transaction(client, tx) {
  const pair = decodeSuiPrivateKey2(process.env.SUI_PRIVATE_KEY || "");
  const keypair = Ed25519Keypair2.fromSecretKey(pair.secretKey);
