...
function hexToNumArray(x) {
  return Array.from((0, import_utils.fromHex)(x));
}
...
function get_arr_bcs_vector(val, deps) {
  if (deps > 0) {
    return import_bcs.bcs.vector(get_arr_bcs_vector(val, deps - 1));
  } else {
...
var Address = class _Address {
  ...
  from_bcs_vector_t(bytes2) {
    return this.from_bcs_vector(import_bcs2.bcs.vector(this.get_bcs()).parse(bytes2));
  }
  serialize(arg) {
  ...
  static get bcs() {
    return import_bcs2.bcs.bytes(32).transform({
      // To change the input type, you need to provide a type definition for the input
      input: (val) => (0, import_utils2.fromHex)(val.into_value()),
      output: (val) => new _Address((0, import_utils2.toHex)(val))
    });
  }
  get_bcs() {
    return _Address.bcs;
...
var Boolean = class _Boolean {
  ...
  static get bcs() {
    return import_bcs2.bcs.bool().transform({
      input: (val) => val,
...
var Ascii = class _Ascii {
  ...
  static get bcs() {
    return import_bcs2.bcs.string().transform({
      input: (val) => val,
...
var U8 = class _U8 {
  ...
  static get bcs() {
    return import_bcs2.bcs.u8().transform({
      input: (val) => val,
...
var U16 = class _U16 {
  ...
  static get bcs() {
    return import_bcs2.bcs.u16().transform({
      input: (val) => val,
...
var U32 = class _U32 {
  ...
  static get bcs() {
    return import_bcs2.bcs.u32().transform({
      input: (val) => val,
...
var U64 = class _U64 {
  ...
  static get bcs() {
    return import_bcs2.bcs.u64().transform({
      input: (val) => val,
      output: (val) => {
        if (!isNaN(Number(val))) {
...
var U128 = class _U128 {
  ...
  static get bcs() {
    return import_bcs2.bcs.u128().transform({
      input: (val) => val,
      output: (val) => {
        if (!isNaN(Number(val))) {
...
var U256 = class _U256 {
  ...
  static get bcs() {
    return import_bcs2.bcs.u256().transform({
      input: (val) => val,
      output: (val) => {
        if (!isNaN(Number(val))) {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      if (r.data) {
        ...
        for (var module_name in moduleMap) {
          ...
          move_gen2.run_module_gen(out, objectid, new Uint8Array((0, import_utils3.fromBase64)(module_base64)));
        }
        ...
        for (var i = 0; i < bcs_json_files.length; i++) {
          ...
          if (bcs_json_data) {
            ...
            for (var bcs_module in bcs_json) {
              runtime.publish_module((0, import_utils4.fromBase64)(bcs_json[bcs_module]));
            }
          }
        }
      }
        ...
        if (import_fs_extra7.default.existsSync(local_bcs_path)) {
          ...
          for (var i = 0; i < bcs_json_files.length; i++) {
            ...
            if (bcs_json_data) {
              var bcs_json = JSON.parse(bcs_json_data);
              for (var bcs_module in bcs_json) {
                runtime.register_module(bcs_json_module, (0, import_utils4.fromBase64)(bcs_json[bcs_module]));
              }
            }
          }
        }
...
async function upgrade_move_module(sui_bin_path, package_path, network, old_package_id, upgrade_cap_id) {
  ...
  const ticket = tx.moveCall({
    ...
    arguments: [
      ...
      tx.pure(import_bcs3.bcs.byteVector().serialize(new Uint8Array(digest)).toBytes())
    ]
  });
